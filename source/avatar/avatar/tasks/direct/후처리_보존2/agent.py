...
class ActorNetwork(nn.Module):
    def __init__(self, node_dim, edge_dim, global_dim, action_dim, max_action, num_gnn_steps=1):
        ...
        self.node_embedder = core.MLP(node_dim, 512, num_layers=7)#5
        self.edge_embedder = core.MLP(edge_dim, 256, num_layers=6)#5
        self.global_embedder = core.MLP(global_dim, 512, num_layers=7)#5

        # 2. GNN Block
        self.gnn = core.RoboBalletGNNBlock(node_dim=512, edge_dim=256, global_dim=512, hidden_dim=256)

        # 3. Action Head
        self.action_head = core.MLP(512, 64, num_layers=2, output_dim=action_dim)
...
class TD3(object):
    def __init__(self, gnn_params, max_action, lr=5e-5, actor_tau=4e-5, critic_tau=8e-5):
        # gnn_params: {node_dim, edge_dim, global_dim, action_dim}
        self.actor = ActorNetwork(**gnn_params, max_action=max_action).to(device)
        self.actor_target = copy.deepcopy(self.actor)
        self.actor_optimizer = torch.optim.Adam(self.actor.parameters(), lr=lr)
    ...
    def train(self, replay_buffer, batch_size, policy_freq=1):
        self.total_it += 1
