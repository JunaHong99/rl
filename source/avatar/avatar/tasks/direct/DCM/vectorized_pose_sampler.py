...
class CachedPoseSampler:
    """
    PoseSampler 캐싱 wrapper.
    - 학습 시작 시 대량 sample을 한 번에 생성 → 파일 저장 → 매 reset 시 random pick
    - PoseSampler 호출이 매 reset마다 일어나면 IK feasibility 검사 등으로 fps 폭락
    - 캐싱하면 reset이 O(1) random index lookup으로 단순화

    파일 저장: pose_cache_<size>.pt
    """
    def __init__(self, device='cuda:0', cache_size: int = 100_000,
        ...
        # Curriculum (2026-06-10 v2): goal 거리 오름차순 정렬 인덱스를 만들어,
        # curriculum_frac으로 sample 가능 범위를 가까운 것부터 점진 확대.
        # 1.0 = 전체 cache 사용 (curriculum off와 동일, 하위호환).
        self.curriculum_frac = 1.0
    ...
    def _load_or_generate(self):
        if os.path.exists(self.cache_path):
            print(f"[CachedPoseSampler] Loading cache from {self.cache_path}")
            cache_cpu = torch.load(self.cache_path, map_location="cpu")
            self.cache = {k: v.to(self.device) for k, v in cache_cpu.items()}
    ...
    def sample_episodes(self, num_envs):
        ...
        if self.curriculum_frac < 1.0 and self.sorted_idxs is not None:
            ...
            min_pool = min(2000, cache_size)
            max_n = max(min_pool, int(self.curriculum_frac * cache_size))
            pick = torch.randint(0, max_n, (num_envs,), device=self.device)
            idxs = self.sorted_idxs[pick]   # 원래 cache 인덱스 (재현성 유지)
        else:
            idxs = torch.randint(0, cache_size, (num_envs,), device=self.device)
        self.last_idxs = idxs  # ★ 마지막 호출의 cache idx (env3에서 per-env idx 추적)
        out = {k: v[idxs].clone() for k, v in self.cache.items()}
        # scalar extras (e.g., obj_width) 그대로 복사
        out.update(self.scalar_extras)
        return out
...
class VectorizedPoseSampler:
    ...
    def __init__(self, device='cuda:0', fixed_grasp_roll: bool = False):
        ...
        self.fixed_grasp_roll = fixed_grasp_roll
    ...
    def sample_episodes(self, num_envs):
        ...
        with torch.no_grad():
            while total_valid < num_envs and loop_cnt < MAX_LOOPS:
                ...
                # Dynamic N: For small requests, don't overkill.
                needed = num_envs - total_valid
                if num_envs <= 64:
                    N = max(needed * 100, 1000) # Small batch for small requests
                else:
                ...
                # Start
                start_off_x = (torch.rand(N, device=self.device) * 0.5) - 0.25 # Expanded to +/- 0.25m
                start_off_y = (torch.rand(N, device=self.device) * 0.8) - 0.4  # Expanded to +/- 0.4m
                start_z = torch.rand(N, device=self.device) * 0.4 + 0.35 # Slightly lowered range
                ...
                # Goal — Stage 2-MAXREACH (2026-06-11): 도달 가능 workspace 전체로 확장.
                #   거리 10~55cm, z변화 ±25cm, yaw start 대비 ±60°.
                #   필터(EE 0.5-0.85m + IK + joint margin)가 도달 불가능분 자동 제거 → "최대 reach"까지만 통과.
                #   (이전 Stage 2: 10-30cm/±15cm/±30°. 97.9% 달성 후 resume 확장용.)
                goal_dist = torch.rand(N, device=self.device) * 0.45 + 0.10   # 10~55cm
                ...
                goal_dz = (torch.rand(N, device=self.device) * 0.50) - 0.25   # ±25cm
                goal_obj_pos = start_obj_pos + torch.stack([goal_dx, goal_dy, goal_dz], dim=1)
                goal_obj_pos[:, 2].clamp_(0.3, 0.8)  # 절대 z 안전 범위
                # Goal yaw — start_yaw 대비 ±60° relative
                goal_yaw_offset = (torch.rand(N, device=self.device) * 2 - 1) * (torch.pi / 3)  # ±60°
                goal_yaw = start_yaw + goal_yaw_offset
                ...
                #          + TCP가 rod_end에 위치하도록 wrist를 +Z(object frame) 방향으로 backoff
                if self.fixed_grasp_roll:
                    grasp_roll = torch.full((N,), torch.pi, device=self.device)
                    z_offset = self.TCP_OFFSET
                else:
                    grasp_roll = torch.rand(N, device=self.device) * 2 * torch.pi
                    z_offset = 0.0

                # Shared Orientation for both grippers (Parallel)
                # Euler(Roll, 0, 0)
                ...
                # ★ Joint margin filter (2026-06-09): start/goal joint angle이 모든 joint
                #   limit에서 ≥ JOINT_MARGIN_THR 떨어져야 함. diagnose_unstable에서 unstable
                #   group의 joint margin이 stable 대비 0.4 vs 0.48로 명확히 낮았고,
                #   top unstable env들이 margin <0.1 (limit 거의 닿음) 자세였음.
                #   IK solver가 q.clamp_(q_min, q_max)로 limit에 박기까지 함 → 추가 필터 필수.
                JOINT_MARGIN_THR = 0.2  # rad (~11.5°)
                q_min = self.ik_solver.q_min  # (7,)
                q_max = self.ik_solver.q_max
                def _min_margin(q):  # q: (B, 7) → (B,) 가장 빠뜻한 joint margin
                    lower = (q - q_min).min(dim=1).values
                    upper = (q_max - q).min(dim=1).values
                    return torch.minimum(lower, upper)
                margin_s1 = _min_margin(q_start_1)
                margin_s2 = _min_margin(q_start_2)
                margin_g1 = _min_margin(q_goal_1)
                margin_g2 = _min_margin(q_goal_2)
                ...
                if num_found > 0:
                    ...
                    collected["q_goal_1"].append(q_goal_1[v_idx])
                    collected["q_goal_2"].append(q_goal_2[v_idx])
                    ...
                    g_ee1_quat = self._quat_mul(g_obj_q[v_idx], off_quat_1[v_idx])
                    g_ee2_quat = self._quat_mul(g_obj_q[v_idx], off_quat_2[v_idx])
        ...
        if total_valid < num_envs:
            ...
            collected["q_goal_1"].append(collected["q_goal_1"][-1][-1:].repeat(missing, 1))
            collected["q_goal_2"].append(collected["q_goal_2"][-1][-1:].repeat(missing, 1))
        ...
        return {
            ...
            "q_goal_1": torch.cat(collected["q_goal_1"]),
            "q_goal_2": torch.cat(collected["q_goal_2"]),
