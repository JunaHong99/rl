...
class DualrobotEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        # current_ee_poses = Arm 노드의 tip(EE) pose — 엣지 상대-pose 계산에 사용.
        raw_state_dict = {
            "current_ee_poses": current_ee_poses, # (B, 2, 7) — Arm tip(EE) pose (엣지용)
            "rod_pos": rod_pos,                   # (B, 3)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if getattr(self.cfg, "use_simul_dist", False):
            # 동시성 거리: 정규화된 나쁜 쪽 좌표(meter 스케일). 뒤처진 좌표 우선 → 동시수렴.
            _K = 0.02 / 0.1745  # POS_T/ROT_T (rot을 pos-등가 스케일로)
            current_dist = torch.maximum(pos_err, _K * rot_err)
        else:
