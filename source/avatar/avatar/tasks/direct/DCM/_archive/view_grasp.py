...
8개를 한 화면에 렌더하면 렉 → N개 env를 멀리 떨어뜨려 한 번에 띄우되, **카메라만 3초씩 이동**해
한 파지씩 비춘다(나머지는 frustum culling으로 렌더 부하 빠짐). env 재생성 없음 → 용접 안정.
...
옵션: --seconds 3(파지당 초), --loop(반복), --same_side, --env_spacing 4. 종료: 뷰포트 닫기/Ctrl-C.
"""
...
parser.add_argument("--seconds", type=float, default=3.0, help="파지당 카메라 표시 시간[s].")
parser.add_argument("--loop", action="store_true", help="끝나면 처음부터 반복.")
parser.add_argument("--env_spacing", type=float, default=4.0, help="env 간 간격[m].")
parser.add_argument("--cache_size", type=int, default=2000, help="초기포즈 캐시 크기(작게=빠름).")
...
# 볼 파지 목록 (d, θ) → N엔트리 preset 저장 (num_envs=N, 라운드로빈이라 env i = 파지 i)
_cfg0 = DualrobotCfg()
...
if args.load:
    ...
    tag = os.path.basename(args.load).replace(".", "_")
    src = os.path.basename(args.load)
else:
    ...
    tag = f"rand_n{args.n}"; src = f"random n={args.n}"

N = int(ds.numel())
preset = f"/tmp/_view_grasp_{tag}{'_ss' if args.same_side else ''}.pt"
torch.save({"d": ds, "theta": ths}, preset)

cfg = DualrobotCfg()
cfg.scene.num_envs = N                  # 파지 수만큼 (라운드로빈 → env i = 파지 i)
cfg.scene.env_spacing = args.env_spacing
cfg.vary_grasp = True
cfg.n_obstacles = 0
cfg.pose_cache_size = args.cache_size
cfg.grasp_same_side = args.same_side
cfg.grasp_preset_path = preset
env = DualrobotEnv(cfg, render_mode=None)
A = env.cfg.action_space
env.reset()
zero_act = torch.zeros(N, A, device=env.device)

d_e = env._grasp_d; th_e = env._grasp_theta; bidx = env._grasp_bucket_idx
print(f"\n===== 파지 {N}개를 {args.seconds:.0f}초씩 카메라 순차 렌더 (src={src}, same_side={args.same_side}) =====")
...
def look_at(i):
    ...
    print(f"  [{i+1}/{N}] bucket={int(bidx[i])}  d={float(d_e[i]):.3f}m  θ={float(th_e[i]):+.3f}rad ({float(th_e[i])*57.3:+.1f}°)")
...
while sim_app.is_running() and (first or args.loop):
    ...
    for i in range(N):
        ...
        while sim_app.is_running() and (time.time() - t0) < args.seconds:
            env.step(zero_act)   # 모든 env 계속 동결 hold, 카메라만 i를 비춤
