...
class SafetyFilter:
    def __init__(self, env):
        ...
        # Fetch limits from robot 1 (assuming identical robots)
        # soft_joint_pos_limits: (num_envs, num_joints, 2)
        # We take the first env and first 7 joints
        ...
        self.k_limit = 5.0      # Reverted to original
        self.limit_margin = 0.1 # Reverted to original
    ...
    def apply_filter(self, actions_1, actions_2):
        ...
        # --- Joint Limit Avoidance (Null Space Repulsion) ---
        # Fetch current joint positions (Arm only)
        # Note: q1, q2 variables above are EE Quaternions, NOT joint angles!
        ...
        q_min_full = self.q_min.repeat(2).unsqueeze(0) # (1, 14)
        q_max_full = self.q_max.repeat(2).unsqueeze(0)
        
        dq_limit = torch.zeros_like(q_curr)
        
        # Lower violation
        dist_lower = q_curr - q_min_full
        mask_lower = dist_lower < self.limit_margin
        dq_limit[mask_lower] = self.k_limit * (self.limit_margin - dist_lower[mask_lower])
        
        # Upper violation
        dist_upper = q_max_full - q_curr
        mask_upper = dist_upper < self.limit_margin
        dq_limit[mask_upper] = -self.k_limit * (self.limit_margin - dist_upper[mask_upper])
        
        # Project onto Null Space: P * dq = dq - J+ (J * dq)
        # This ensures limit avoidance doesn't break the closed-chain constraint
        if torch.any(dq_limit != 0):
            J_dq_limit = torch.bmm(J_c, dq_limit.unsqueeze(2))
            dq_limit_projected = dq_limit.unsqueeze(2) - torch.bmm(J_dagger, J_dq_limit)
            q_safe = q_safe + dq_limit_projected.squeeze(2)
        # ----------------------------------------------------
