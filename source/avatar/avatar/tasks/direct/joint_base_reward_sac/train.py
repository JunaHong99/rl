...
def main():
    ...
    # 학습률 등 하이퍼파라미터 설정
    # agent = TD3(gnn_params=gnn_params, max_action=1.0, lr=5e-5) #5e-5면 로보발레와 동일한 수치
    agent = SAC(gnn_params=gnn_params, max_action=1.0, lr=3e-4) # SAC Default LR
    ...
    for step in range(start_step, args_cli.max_iterations):
        ...
        if step < WARMUP_STEPS:
            ...
            with torch.no_grad():
                ...
                # SAC: Sample action (already has stochasticity)
                full_actions, _, _ = agent.actor.sample(current_batch_graph)
                ...
                # SAC handles noise via sampling, so we don't add manual noise here
                # unless we want to clip strictly again, but tanh already keeps it in range.
        ...
        # -------------------------------------------------
        # 버퍼가 어느 정도 차면 학습 시작
        # [수정] 희소 보상 태스크를 위한 재사용 빈도 상향
        # - BATCH_SIZE: 2048 (대규모 배치로 성공 샘플 포함 확률 확보)
        # - TARGET_SIR: 3.0 (데이터 1개당 3회 재사용 -> 희소한 성공 경험 학습 강화)
        TARGET_SIR = 3.0 
        BATCH_SIZE = 2048 
        if step >= WARMUP_STEPS:
            ...
            for _ in range(gradient_steps): #버퍼에서 샘플링하여 업데이트
                metrics = agent.train(buffer, batch_size=BATCH_SIZE)
            ...
            if steps_count > 0 and step % 100 == 0:
                for k, v in agg_metrics.items():
                    writer.add_scalar(k, v / steps_count, step)
