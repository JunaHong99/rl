...
#시작, 목표 ee 위치가 로봇 베이스로부터 40cm 이상 떨어지도록 제한해놨음.
class VectorizedPoseSampler:
    ...
    def sample_episodes(self, num_envs):
        ...
        # [NEW] Distance Check (EE vs Base > 0.3m)
        d_s1 = torch.norm(ee1_w_p - b1_p, dim=1)
        d_s2 = torch.norm(ee2_w_p - b2_p, dim=1)
        ...
        # [NEW] Goal Distance Check
        d_g1 = torch.norm(g_ee1_w_p - b1_p, dim=1)
        d_g2 = torch.norm(g_ee2_w_p - b2_p, dim=1)
        ...
        valid_mask = (s1_p_err < POS_THR) & (s1_r_err < ROT_THR) & \
                     ...
                     (d_s1 > DIST_THR) & (d_s2 > DIST_THR) & \
                     (d_g1 > DIST_THR) & (d_g2 > DIST_THR)
        ...
        return {
            ...
            "q_goal_1": q_goal_1[valid_indices],
            "q_goal_2": q_goal_2[valid_indices],
