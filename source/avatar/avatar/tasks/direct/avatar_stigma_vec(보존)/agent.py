...
class TD3(object):
    ...
    #         # Actor 출력은 [Total Nodes, Dim] 이므로 로봇 부분만 필터링 및 Reshape 필요
    #         # 단순화를 위해 전체 출력 사용하되 Critic에서 잘라서 씀
    #         next_full_action = self.actor_target(next_state)
            
    #         # Reshape to [Batch, Robots, Dim]
    #         # 여기서는 로봇 수 2로 하드코딩 (Pipeline Check용)
    #         n_robots = 2
    #         next_robot_action = next_full_action.reshape(batch_size, -1, action.shape[-1])[:, :n_robots, :]

    #         # Add Noise
    #         noise = (torch.randn_like(next_robot_action) * 0.2).clamp(-0.5, 0.5)
    #         next_action = (next_robot_action + noise).clamp(-self.max_action, self.max_action)
    ...
    #     actor_action = full_actor_action.reshape(batch_size, -1, action.shape[-1])[:, :n_robots, :]
