...
class DualrobotEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # (무한대에서 빼면 NaN이나 엄청 큰 값이 나오므로 방지)
        is_first_step = torch.isinf(self.prev_dist)
        self.prev_dist = torch.where(is_first_step, current_dist, self.prev_dist)

        # Progress: (과거 거리 - 현재 거리) => 가까워지면 (+), 멀어지면 (-)
        # Scale Factor: 100.0 (변화량이 작으므로 키워줌)
        progress = (self.prev_dist - current_dist) * 100.0
        
        # [중요] 리워드 해킹 방지 1: 진동(Oscillation) 방지
        # 뒤로 가는 것에 대한 페널티를 그대로 둠 (Clamping 하지 않음). 
        # Symmetric하게 처리하여 제자리 왕복 시 이득이 없게 만듦.
        ...
        # ---------------------------------------------------------
        # 리워드 해킹 방지 2: 지연(Procrastination) 방지
        # 포텐셜 보상만 있으면 천천히 가도 총합이 같으므로, 빨리 가도록 재촉함.
        r_time = -0.1
        ...
        w_action = 0.05 # 기존 0.1에서 약간 완화 (움직임 장려)
        ...
        r_success = torch.where(is_truly_success, 50.0, 0.0) # 성공 보상 강화 (20 -> 50)

        # 가중 합산
        # r_potential: 움직이는 방향성 제시
        # r_time: 속도 독려
        # r_constraint: 안전장치
        ...
        self.extras["log/r_potential"] = torch.mean(r_potential) # Log Name Change
