...
# 1. Helper Function
# ---------------------------------------------------------
def convert_batch_obs_to_graph_list(raw_obs_dict, num_envs):
    """Isaac Lab의 배치 텐서를 개별 PyG Data 객체 리스트로 변환"""
    graph_list = []
    keys = raw_obs_dict.keys()
    
    for i in range(num_envs):
        single_env_obs = {k: raw_obs_dict[k][i] for k in keys}
        graph_data = convert_state_to_graph(single_env_obs)
...
def main():
    ...
    # 성공률 계산을 위한 이동 평균 버퍼 (최근 100개 에피소드)
    ...
    # 대용량 버퍼 (RAM 여유에 따라 조절)
    buffer = GraphReplayBuffer(capacity=1000000) 
    
    # --- C. 학습 루프 시작 ---
    obs_dict, _ = env.reset()
    current_graphs = convert_batch_obs_to_graph_list(obs_dict['policy'], args_cli.num_envs)
    ...
    for step in range(start_step, args_cli.max_iterations):
        ...
        # -------------------------------------------------
        batch_graph = Batch.from_data_list(current_graphs).to(device)
        
        agent.actor.eval()

        with torch.no_grad():
            # GNN 출력: [Total_Nodes, 7] 
            full_actions = agent.actor(batch_graph)
        ...
        # -------------------------------------------------
        next_graphs = convert_batch_obs_to_graph_list(next_obs_dict['policy'], args_cli.num_envs)
        
        # CPU로 이동 (버퍼 저장용)
        actions_cpu = env_actions_tensor.cpu()
        rewards_cpu = rewards.cpu()
        dones_cpu = dones.cpu()
        
        for i in range(args_cli.num_envs):
            buffer.add(
                state_graph=current_graphs[i],
                action=actions_cpu[i],
                next_state_graph=next_graphs[i],
                reward=rewards_cpu[i].item(),
                done=dones_cpu[i].item()
            )
        ...
        if len(buffer) > 100000: #10000
            agent.train(buffer, batch_size=256)#256
            #print('정상 학습 진행 중')
       ...
       # -------------------------------------------------
        ...
        if step % 100 == 0:
            ...
            # (1) 기본 기록
            writer.add_scalar("Rollout/AvgReward", mean_reward, step)

            # (2) 리워드 성분별 기록 (수정됨)
            # extras에 키가 존재하는지 확인 후 기록
            if "log/total_reward" in extras:
                writer.add_scalar("Reward/Total", extras["log/total_reward"].item(), step)
                ...
                #writer.add_scalar("Reward/Time", extras["log/r_time"].item(), step)
                # 보너스 및 패널티
                writer.add_scalar("Reward/Success", extras["log/r_success"].item(), step)
            ...
            if "log/curr_threshold" in extras:
                writer.add_scalar("Rollout/threshold", extras["log/curr_threshold"].item(), step)
