...
class GNNActor(nn.Module):
    ...
    def __init__(
        ...
        # Head: rod_embedding + global → action
    ...
    def forward(self, batch):
        ...
        B = batch.num_graphs
        rod_x = self._extract_rod_features(x, B)               # (B, node_dim)
        h = torch.cat([rod_x, u], dim=-1)                      # (B, node_dim + global_dim)
...
class GNNQCritic(nn.Module):
    ...
    def forward(self, batch, action):
        ...
        from torch_scatter import scatter_mean
        x_pool = scatter_mean(x, batch.batch, dim=0)  # (B, node_dim)
        h = torch.cat([x_pool, u, action], dim=-1)
