...
ROD_CFG = RigidObjectCfg(
    ...
    spawn=sim_utils.CuboidCfg(
        ...
        rigid_props=sim_utils.RigidBodyPropertiesCfg(
            kinematic_enabled=False,           # Phase 2: dynamic
            disable_gravity=True,              # 중력은 일단 OFF (안정성 우선)
            max_depenetration_velocity=5.0,
        ),
        mass_props=sim_utils.MassPropertiesCfg(mass=0.1),  # 가벼운 막대
        # Phase 2: rod는 fixed joint로 그리퍼에 강결합되므로 collision 불필요
        # contact pair 추적을 끄면 GPU 메모리 사용량 대폭 감소
...
class DualrobotCfg(DirectRLEnvCfg):
    ...
    # === 1. 환경 기본 설정 ===
    decimation = 4 # 물리연산 4번당 1번 제어 (240Hz / 4 = 60Hz)
    episode_length_s = 20.0 #총 에피소드 스텝: 시뮬레이션 주파수/decimation * 시간(episode_length)
