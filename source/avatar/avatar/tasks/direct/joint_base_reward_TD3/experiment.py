...
def main():
    ...
    if os.path.exists(args_cli.dataset_file):
        ...
        # [Validation] Check size compatibility and required keys
        loaded_size = dataset["base_pose_1"].shape[0]
        has_required_keys = "q_goal_1" in dataset and "q_goal_2" in dataset
        
        if loaded_size != args_cli.num_envs or not has_required_keys:
            if loaded_size != args_cli.num_envs:
                print(f"⚠️ Dataset size mismatch! Loaded: {loaded_size}, Requested: {args_cli.num_envs}")
            if not has_required_keys:
                print(f"⚠️ Dataset is missing required keys (q_goal_1, q_goal_2).")
            
            if loaded_size > args_cli.num_envs and has_required_keys:
                print(f"📉 Slicing dataset to match {args_cli.num_envs}...")
                for k in dataset:
                    if isinstance(dataset[k], torch.Tensor):
                        dataset[k] = dataset[k][:args_cli.num_envs]
            else:
                print(f"📈 Regenerating {args_cli.num_envs} samples...")
    ...
    external_samples = {
        ...
        "q_goal_1": dataset["q_goal_1"],
        "q_goal_2": dataset["q_goal_2"],
    ...
    # [NEW] Global Max Error Tracking per Env
    global_max_pos_error = torch.zeros(env.num_envs, device=device)
    global_max_rot_error = torch.zeros(env.num_envs, device=device)
    
    # 에피소드 길이만큼 실행 (환경의 max_length 사용)
    max_steps = 1200 # 혹시 모르니 하드코딩 혹은 env.max_episode_length
    ...
    for step in range(max_steps):
        ...
        # Define dones immediately
        dones = terminated | truncated
        
        if (step + 1) % 10 == 0:
            print(f"Step {step+1}/{max_steps} completed.")
        
        # [NEW] Update Max Errors
        # Direct access to env's internal buffer for per-env max error
        current_max_pos = env.episode_max_pos_error
        current_max_rot = env.episode_max_rot_error
        
        global_max_pos_error = torch.max(global_max_pos_error, current_max_pos)
        global_max_rot_error = torch.max(global_max_rot_error, current_max_rot)
    ...
    print("-" * 30)
    print("Constraint Error Stats:")
    print(f"  - Worst Case Pos Error: {torch.max(global_max_pos_error).item():.4f} m")
    print(f"  - Worst Case Rot Error: {torch.max(global_max_rot_error).item():.4f} rad")
    print(f"  - Avg Max Pos Error   : {torch.mean(global_max_pos_error).item():.4f} m")
    print(f"  - Avg Max Rot Error   : {torch.mean(global_max_rot_error).item():.4f} rad")
    print("-" * 30)
