...
def main():
    ...
    # [1] Sort indices: 'timeout' first
    sorted_indices = []
    timeout_idxs = [i for i, r in enumerate(reasons) if r == "timeout"]
    other_idxs = [i for i, r in enumerate(reasons) if r != "timeout"]
    sorted_indices = timeout_idxs + other_idxs
    
    print(f"Sorted: {len(timeout_idxs)} 'timeout' cases first, then {len(other_idxs)} others.")
