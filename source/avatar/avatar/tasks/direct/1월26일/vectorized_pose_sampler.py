...
#             # --- C. Offsets Generation ---
#             half_width = obj_width / 2.0
#             noise_range = 0.05
            
#             r_noise1 = (torch.rand(N, device=self.device) * 2 - 1) * noise_range
#             p_noise1 = (torch.rand(N, device=self.device) * 2 - 1) * noise_range
#             y_noise1 = (torch.rand(N, device=self.device) * 2 - 1) * noise_range
#             off_pos_1 = torch.tensor([-half_width, 0.0, 0.0], device=self.device).repeat(N, 1)
#             off_quat_1 = self._euler_to_quat(torch.zeros(N, device=self.device)+r_noise1, 
#                                              torch.ones(N, device=self.device)*np.pi/2 + p_noise1, 
#                                              torch.zeros(N, device=self.device)+y_noise1)

#             r_noise2 = (torch.rand(N, device=self.device) * 2 - 1) * noise_range
#             p_noise2 = (torch.rand(N, device=self.device) * 2 - 1) * noise_range
#             y_noise2 = (torch.rand(N, device=self.device) * 2 - 1) * noise_range
#             off_pos_2 = torch.tensor([half_width, 0.0, 0.0], device=self.device).repeat(N, 1)
#             off_quat_2 = self._euler_to_quat(torch.zeros(N, device=self.device)+r_noise2, 
#                                              torch.ones(N, device=self.device)*(-np.pi/2) + p_noise2, 
#                                              torch.zeros(N, device=self.device)+y_noise2)

#             # --- D. Distance Filter (EE Based) ---
#             # Calculate EE world positions
#             s_ee1_p, _ = self._get_ee_pose_world(start_obj_pos, start_obj_quat, off_pos_1, off_quat_1)
#             s_ee2_p, _ = self._get_ee_pose_world(start_obj_pos, start_obj_quat, off_pos_2, off_quat_2)
#             g_ee1_p, _ = self._get_ee_pose_world(goal_obj_pos, goal_obj_quat, off_pos_1, off_quat_1)
#             g_ee2_p, _ = self._get_ee_pose_world(goal_obj_pos, goal_obj_quat, off_pos_2, off_quat_2)

#             d1_start = torch.norm(s_ee1_p - base_pos_1, dim=1)
#             d2_start = torch.norm(s_ee2_p - base_pos_2, dim=1)
#             d1_goal = torch.norm(g_ee1_p - base_pos_1, dim=1)
#             d2_goal = torch.norm(g_ee2_p - base_pos_2, dim=1)
            
#             dist_thr = 0.40
#             valid_mask = (d1_start > dist_thr) & (d1_start < 0.80) & \
#                         (d2_start > dist_thr) & (d2_start < 0.80) & \
#                         (d1_goal > dist_thr) & (d1_goal < 0.80) & \
#                         (d2_goal > dist_thr) & (d2_goal < 0.80)
...
#                 collected["offset_pos_1"].append(off_pos_1[indices])
#                 collected["offset_quat_1"].append(off_quat_1[indices])
#                 collected["offset_pos_2"].append(off_pos_2[indices])
#                 collected["offset_quat_2"].append(off_quat_2[indices])
